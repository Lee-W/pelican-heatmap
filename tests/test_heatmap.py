...
def make_generator(articles: list, output_path: str):
    """Return a minimal mock ArticlesGenerator."""
    return SimpleNamespace(articles=articles, output_path=output_path)


def run_generate(articles: list):
    """Run generate_heatmap and return the parsed JSON output."""
    with tempfile.TemporaryDirectory() as tmpdir:
        generator = make_generator(articles, tmpdir)
        generate_heatmap(generator)
...
class TestJSONStructure:
    ...
    def test_no_articles_produces_no_file(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            generator = make_generator([], tmpdir)
            generate_heatmap(generator)
            assert not os.path.exists(os.path.join(tmpdir, "writing-heatmap.json"))

    def test_output_dir_created_if_missing(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            output_path = os.path.join(tmpdir, "nested", "output")
            articles = [make_article("A", "/a/", "2025-01-01")]
            generator = make_generator(articles, output_path)
            generate_heatmap(generator)
...
class TestStreak:
    ...
    def _run(self, data: dict, today_str: str):
